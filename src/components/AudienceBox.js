          ...
          querySnapshot.forEach(async (doc) => {
            ...
            if (querySnapshot.size == counter)
            {
                if(bubbleMembersArray.length == 1)
                    ...
                    setStatusForMessage("Status for: " + bubbleMembersArray[0] + ", " + bubbleMembersArray[1]);

                else
                    setStatusForMessage("Status for: " + bubbleMembersArray[0] + ", " + bubbleMembersArray[1] + ", and " + (bubbleMembersArray.length-1).toString() + " more");
...
{/* TO DO: Add exlude functionality */}
                <TouchableWithoutFeedback onPress={() => setExcludePersonVisible(!isExcludePersonVisible)}>
                    <View>
                            ...
                            <Text style={audienceBoxStyles.audienceBoxNameListText}>{statusForMessage}</Text>
